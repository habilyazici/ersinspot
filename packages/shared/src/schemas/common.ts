...
import { z } from 'zod';
import { IZMIR_DISTRICTS, SERVICED_DISTRICTS } from '../domain/locations.ts';
import { normalize as normalizePhone } from '../domain/phone.ts';
...
/**
 * Kullanıcıdan gelen tüm serbest metinlerde kullanılır: baştaki/sondaki boşlukları
 * kırpar ve görünmez kontrol karakterlerini temizler.
 */
const cleanText = (value: string): string =>
  value
    .trim()
    // C0/C1 kontrol karakterleri, sıfır genişlikli birleştiriciler ve yön değiştirme
    // işaretleri: görünmez oldukları için doğrulamayı atlatmak, arama sonuçlarını
    // bozmak veya ekranda metni ters çevirmek amacıyla kullanılabilirler.
...
export function requiredText(field: string, min = 1, max = 500) {
  return z
    .string({ required_error: `${field} zorunludur.`, invalid_type_error: `${field} metin olmalıdır.` })
    .transform(cleanText)
    .pipe(
      z
        .string()
